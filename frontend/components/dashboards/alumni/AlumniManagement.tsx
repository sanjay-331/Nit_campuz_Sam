...
import { Alumnus, UserRole, StudentStatus } from '../../../types';
...
const AlumniManagement: React.FC = () => {
    ...
    const ALUMNI = useMemo(() => users.filter((u: any) => u.role === UserRole.STUDENT && u.status === StudentStatus.ALUMNI) as Alumnus[], [users]);
    ...
    const graduationYears = useMemo(() => Array.from(new Set(ALUMNI.map(a => Number(a.graduationYear)))).sort((a: number, b: number) => b - a), [ALUMNI]);
    ...
    const handleConfirmExport = () => {
        ...
        dispatch(showToast({ type: 'success', message: `${selectedAlumni.length} alumni records exported successfully.` }));
        setSelectedAlumni([]);
    };
